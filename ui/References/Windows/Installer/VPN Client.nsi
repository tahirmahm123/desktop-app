...
!define PRODUCT_NAME "VPN Client"
!define PRODUCT_IDENTIFIER "VPN Client"
!define PRODUCT_PUBLISHER "VPN Limited"

!define APP_RUN_PATH "$INSTDIR\ui\VPN Client.exe"
!define PROCESS_NAME "VPN Client.exe"
!define VPN_SERVICE_NAME "VPN Client"
!define PATHDIR "$INSTDIR\cli"

!define DEVCON_BASENAME "devcon.exe"
!define PRODUCT_TAP_WIN_COMPONENT_ID "tapvpn"
...
!define MUI_FINISHPAGE_RUN "$INSTDIR\VPN Client.exe"
...
CreateShortcut "$DESKTOP\VPN Client.lnk" "${APP_RUN_PATH}"
FunctionEnd
...
Section "${PRODUCT_NAME}" SecVPN
  ...
  ReadRegStr $R1 HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\VPN Client" "DisplayVersion"
  ${StrLoc} $R2 $R1 "2.12." ">"
  StrCmp $R2 "0" done ; R2 must be 0 if upgrading from '2.12.X' version

  ReadRegStr $R0 HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\VPN Client" "UninstallString"
  StrCmp $R0 "" done
  ...
  Pop $0 ; 1 - SUCCESS;
  ${if} $0 != 1
		DetailPrint "ERROR: Failed to stop 'VPN Client' service."
		MessageBox MB_ABORTRETRYIGNORE|MB_ICONEXCLAMATION "Failed to stop 'VPN Client' service.$\nIgnoring this problem can lead to issues with VPN Client software in the future." IDRETRY stopservcice IDIGNORE ignoreservicestop
		DetailPrint "Aborted"
		Abort
  ${EndIf}
  ignoreservicestop:
  ...
  Pop $0 ; 1 - SUCCESS
  ${if} $0 != 1
		DetailPrint "ERROR: Failed to stop 'VPN Client' application."
		MessageBox MB_ABORTRETRYIGNORE|MB_ICONEXCLAMATION "Failed to stop 'VPN Client' application.$\nIgnoring this problem can lead to issues with VPN Client software in the future." IDRETRY stopclient IDIGNORE ignoreclientstop
		DetailPrint "Aborted"
		Abort
  ${EndIf}
  ignoreclientstop:
  ...
  Push "$INSTDIR\VPN Firewall Native x64.dll" ; file to check for writting
  Push 15000 ; 15 seconds
  Call WaitFileOpenForWritting

  ; check is library can be overwritten
  Push "$INSTDIR\VPN Helpers Native x64.dll" ; file to check for writting
  Push 15000 ; 15 seconds
  Call WaitFileOpenForWritting
  ...
  CreateShortCut "$SMPROGRAMS\$StartMenuFolder\${PRODUCT_NAME}.lnk" "$INSTDIR\ui\VPN Client.exe"
  ...
  */
  ...
  nsExec::ExecToLog '"$SYSDIR\sc.exe" create "VPN Client" binPath= "\"$INSTDIR\VPN Service.exe\"" start= auto'
  nsExec::ExecToLog '"$SYSDIR\sc.exe" sdset "VPN Client" "D:(A;;CCLCSWRPWPDTLOCRRC;;;SY)(A;;CCDCLCSWRPWPDTLOCRSDRCWDWO;;;BA)(A;;CCLCSWLOCRRC;;;IU)(A;;CCLCSWLOCRRC;;;SU)(A;;RPWPDTLO;;;S-1-1-0)"'
  ...
  nsExec::ExecToLog '"$SYSDIR\sc.exe" start "VPN Client"'
SectionEnd
...
Section "Uninstall"
  ...
  ; remove service
  nsExec::ExecToLog '"$SYSDIR\sc.exe" delete "VPN Client"'
...
Function StopService
	DetailPrint "Checking is VPN Client service is running..."
	...
	DetailPrint "Stopping VPN Client service..."
