...
:build_agent_plat
	...
	go build -tags release -o "bin\%~1\IVPN Service.exe" -trimpath -ldflags "-s -w -X github.com/tahirmahm123/vpn-desktop-app/daemon/version._version=%APPVER% -X github.com/tahirmahm123/vpn-desktop-app/daemon/version._commit=%COMMIT% -X github.com/tahirmahm123/vpn-desktop-app/daemon/version._time=%DATE%" || exit /b 1
