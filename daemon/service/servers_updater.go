...
type serversUpdater struct {
	servers           *types.ServerListResponse
	api               *api.API
...
func (s *serversUpdater) GetServers() (*types.ServerListResponse, error) {
	if s.servers != nil {
...
func (s *serversUpdater) GetServersForceUpdate() (*types.ServerListResponse, error) {
	return s.updateServers()
}
...
func readServersFromCache() (svrs *types.ServerListResponse, e error) {
	...
	if err := json.Unmarshal(data, servers); err != nil {
		return nil, fmt.Errorf("failed to unmarshal servers cache file: %w", err)
	}
	...
	if err := filerights.CheckFileAccessRightsConfig(serversFile); err != nil {
		...
		// It is safe, because we are checking TLS server name for "api.ivpn.net" when accessing API (https)
		return nil, fmt.Errorf("skip reading servers cache file: %w", err)
	}

	return servers, nil
}

func writeServersToCache(servers *types.ServerListResponse) error {
	if servers == nil {
